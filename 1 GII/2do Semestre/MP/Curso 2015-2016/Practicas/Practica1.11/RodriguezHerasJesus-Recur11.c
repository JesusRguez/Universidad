...
//Postcondición: Inicializa la función recursiva
int llamada(int* A, int n)              //Función llamada a la llamada recursiva
{
    ...
    return(cambio(A,n,0,0,ta));         //Llamada a la función recursiva con los parámetros A (vector), n (número de elementos del vector), i (indice del vector inicializado a 0), cambios (número de cambios de tendencia inicializado a 0) y ta (parámetro con la tendencia actual que se inicializará en función de la comparación llevada a cabo en la función llamada con el primer y segundo elemento del vector
}
...
{
    if (i==(n-1))
        return cambios;                                     //Si el índice del vector es igual al máximo número de elementos del vector, la función termina deevolviendo el número de cambios actal, es decir, 0, si no es así continuará con el bloque else
    else{
        if (A[i]>A[i+1] && ta=='+')
