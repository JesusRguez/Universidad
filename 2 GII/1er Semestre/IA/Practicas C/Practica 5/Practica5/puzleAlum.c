...
int mal_colocadas(tEstado *actuaal, tEstado *fin)   //Mira qué piezas están mal colocadas y lo utiliza como función heurística
{
    int i,j,mal=0;
    ...
    for(i=0;i<N;i++)
    {
        for(j=0;j<N;j++)
        {
            if (actuaal->celdas[i][j]!=fin->celdas[i][j])
                mal++;
        }
    }
    return mal;
...
void ordena(Lista C, tNodo *elem)       //Ordena una lista de elementos
{
    int i=0,j=0;
    tNodo *extraido=(tNodo*)malloc(sizeof(tNodo));
    tNodo *ultimo=(tNodo*)malloc(sizeof(tNodo));
    ultimo=ExtraerElem(C,C->Lmax);
    extraido=ExtraerElem(C,i);
    while (elem->valHeuristica<extraido->valHeuristica)
    {
        i++;
        extraido=ExtraerElem(C,i);

    }
    SeparaLista(C, i, elem);

}

void SeparaLista(Lista C, int i, tNodo *elem)   //Separa la lista de los elementos y nos permite introducir el nuevo nodo con menor heurística
{

    Lista *PCacho=(Lista*)malloc(sizeof(Lista));
    memcpy(PCacho, C, (i+1)*sizeof(tNodo));
    while(i<C->Nelem)
    {
        i++;
        InsertarUltimo(elem, PCacho);
        ExtraerElem(C, i+1);
    }
}

int Distancia_Manhattan(tEstado *estado, tEstado *objetivo)     //Calcula la distancia de Manhattan como función heurística
{
    int i,j,a,b,total;
    total=0;
    for(i=1;i<N*N;i++)
    {
        a=abs(estado->col[i]-objetivo->col[i]);
        b=abs(estado->fila[i]-objetivo->fila[i]);
        total=total+a+b;
    }
    return total;
}
