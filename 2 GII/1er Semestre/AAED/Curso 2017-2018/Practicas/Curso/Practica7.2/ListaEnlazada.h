...
template <typename T> inline Lista<T>::Lista() : L(new nodo(T())) // crear cabecera
{}
...
template <typename T> inline void Lista<T>::insertar(const T& x)
{
    Lista<T>::posicion p=primera();
    while(p->sig!=nullptr && p->sig->elto < x){ //Como C++ opera en conrtocircuito, en cuanto ve que p->sig!=nullptr es false, pasa de evaluar la segunda condición del while y continua la ejecución.
        p=p->sig;
    }
...
template <typename T> inline void Lista<T>::eliminar(Lista<T>::posicion p)
{
...
template <typename T> typename Lista<T>::posicion Lista<T>::anterior(Lista<T>::posicion p) const
{
    nodo* q;
...
template <typename T> typename Lista<T>::posicion Lista<T>::fin() const
{
    nodo* p;
