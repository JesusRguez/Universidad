 ...
 * a base de m anguilas cocinadas al eneldo. Cuando un vikingo quiere comer, se
 * sirve una anguila. Si ya no quedan, avisa al vikingo cocinero para que
 * proceda a llenar la marmita de nuevo, utilizando las inagotables provisiones
 * de anguilas disponibles. Desarrolle un programa en java que modele esta
 * curiosa situación, y provea la sincronización necesaria utilizando semáforos
 * de clase java.util.concurrent.Semaphore. Llame a su programa
 * drakkarVikingo.java.
 */
        ...
        switch (tipoTripulante) {
            case CONCINERO:
                while (true) {
                    ...
                    } // Wait Exclusión mútua para acceso a la Marmita para rellenar

                    try {
                        SemMutex.acquire();
                    } catch (InterruptedException e) {
                    } // Wait Exclusión mútua para acceso a índice de relleno de la Marmita
            ...
            case TRIPULANTE:
                while (true) {
                    try {
                        SemTripulante.acquire();
                    } catch (InterruptedException e) {
                    } // Wait Sincronización con Cocinero: Primero llenar marmita, luego comer

                    try {
                        SemMutex.acquire();
                    } catch (InterruptedException e) {
                    } // Wait Exclusión mútua para acceso a índice de consumo de la Marmita
