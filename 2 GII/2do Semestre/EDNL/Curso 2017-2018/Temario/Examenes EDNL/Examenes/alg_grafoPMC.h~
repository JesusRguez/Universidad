...
template<typename tCoste>
using verticeP = typename GrafoP<tCoste>::vertice;

template <typename tCoste>
vector<tCoste> DijkstraInv(const GrafoP<tCoste>& G,
                           verticeP<tCoste> destino,
                           vector<verticeP<tCoste> >& P)
{
    using vertice = verticeP<tCoste>;
    ...
    // Iniciar D y P con caminos directos desde la columna del destino.
    for(int i = 0; i < n; ++i)
        D[i] = G[i][destino];
