...
template <typename T> inline Abin<T>::Abin(size_t maxNodos) : nodos(new celda[maxNodos]), maxNodos(maxNodos), numNodos(0) {}
...
template <typename T> int Abin<T>::profundidad(typename Abin<T>::nodo n){
    if(n==raizB())
        return 0;
    else
...
template <typename T> int Abin<T>::altura(typename Abin<T>::nodo n){
    if(n==NODO_NULO){
        return -1;
    }else{
...
template <typename T> int Abin<T>::maximo(int n, int m){
    if(n<m){
        return m;
    }else{
        return n;
