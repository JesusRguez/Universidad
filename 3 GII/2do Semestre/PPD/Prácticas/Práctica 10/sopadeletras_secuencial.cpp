...
int main(){
			...
			for(int j=0;j<N;++j){
				if(v[k][l]==sopa[i][j]){
					if(busca(sopa,i,j,v[k],&sentido)){
						x=i;
						y=j;
						//printf("Palabra %s en la posicion (%d,%d) con sentido %s\n",v[k],x,y,(sentido? "vertical":"horizontal"));
						cout<<"Palabra "<<v[k]<<" en la posicion ["<<x<<"], ["<<y<< "] con sentido "<<(sentido? "vertical":"horizontal")<<endl;
					}
				}
			}
	...
	cout<<"Tiempo de búsqueda secuencial: "<<time<<" segundos."<<endl;
